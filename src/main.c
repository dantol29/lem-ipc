...
static void *get_shared_memory()
{
    ...
    if (key == -1)
        exit_error("Could not create a key");
    ...
    else if (state.shared_memory_id == -1)
        exit_error("Could not create shared memory");

    void *shared_memory = shmat(state.shared_memory_id, 0, 0); // attach shared memory to the page table of the process
    if (shared_memory == (void *)-1)
        exit_error("Could not map shared memory");
...
int main(int argc, char **argv)
{
    ...
    if (state.semaphores_id == -1)
    {
        cleanup_resources();
        exit_error("Semaphore");
    }
    ...
    if (player_pos != -1)
        player_loop(shared_memory, team, player_pos);
